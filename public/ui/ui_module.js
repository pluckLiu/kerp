...
var CommonButton = k.react.CommonButton = React.createClass({
    ...
    btnClick: function (event) {
        ...
        if (btn.is('.noneButton')) {
            ...
            var end = this.props.parentClick(this.state.jsFn, event, this.props.parentCtrType == "MxPage" ? this.props.proc : this.props.proc == null || this.props.proc == '' ? false : true, this.props.btid, this.state.text);
            //如果是修改保存按钮
    ...
    render: function () {
        ...
        if (className == undefined) className = "kCommonButton";
        return React.createElement(
            'a',
            { className: className, id: this.props.dsid, onClick: this.btnClick },
            this.state.text
        );
    }
}); //通用按钮
var SearchProgress = k.react.SearchProgress = React.createClass({
    ...
    render: function () {
        ...
        var mainStyle = {
            display: this.state.totalCount == 0 ? 'none' : this.state.isShow == false ? 'none' : 'block', opacity: 1
        }; //是否显示
        ...
        return React.createElement(
            'div',
            { className: "kSearchProgress", style: mainStyle },
            React.createElement('span', { className: "kProgressGreen", style: progressStyle }),
            React.createElement(
                'span',
                {
                    className: "kSearchProgressText" },
                this.state.nowRows.toString() + '/' + this.state.totalCount
            )
        );
    }
}); //查询进去条
var DsSearchPage = k.react.DsSearchPage = React.createClass({
    getDsSearchBtns: function () {
        ...
        for (var i = 0, len = searchBtns.length; i < len; i++) {
            if (searchBtns[i]['JS_FUNC'] == 'exportExcelByNode' || searchBtns[i]['JS_FUNC'] == 'uploadDataToExcel') isNewExportExists = true;
        }
        if (isNewExportExists == false) searchBtns.push({
        ...
        for (var i = 0, len = searchBtns.length; i < len; i++) {
            btn = searchBtns[i];
            DsSearchBtns.push(React.createElement(CommonButton, { key: btn['NAME'] + i.toString(),
    ...
    btnClick: function (fn, event, isProc, btid, text) {
        ...
        if (btid.indexOf('bigProc') >= 0) this['bigProc'](event.target, isProc, btid, text);else if (this[fn]) this[fn](event.target, isProc, btid, text);else $(event.target).removeClass('noneButton');
    },
            ...
            k.CallServerAPI('bigProc', {
                ...
                if (data.err != undefined) {
                    ...
                    try {
                        ...
                        if (json.msg != undefined) alert(json.msg);else if (json.error != undefined) alert(json.error);else if (json.jsFn != undefined) {
                            //如果有自定义函数
                            var fn = new Function(json.jsFn);
                            if (k.isFunction(fn)) fn.apply(that); //通过apply 压入 SearchPage
                        }
                    } catch (e) {
                        alert(data.outProc);
                    }
        ...
        if (selectRows.length == 0) {
            ...
            k.CallServerAPI('djProc', {
                ...
                if (data.err != undefined) {
                    ...
                    try {
                        var json = JSON.parse(data.outProc); //格式化json
                        if (json.msg != undefined) alert(json.msg);else if (json.jsFn != undefined) {
                            //如果有自定义函数
                            var fn = new Function(json.jsFn);
                            if (k.isFunction(fn)) fn.apply(that); //通过apply 压入 SearchPage
                        }
    ...
    save: function (btn) {
        ...
        k.CallServerAPI('saveds', {
            ...
            k.flux.emitAction('DataGrid', 'SearchPage' + '-' + that.props.mid + '-' + that.props.dsid, 'ClearEditData');
    ...
    render: function () {
        ...
        return React.createElement(
            'div',
            { id: this.props.dsid, className: classes },
            React.createElement(
                'div',
                { className: 'kSearchPanel', style: kSearchPanelStyle },
                React.createElement(
                    'div',
                    { className: 'kSearchBtnsPanel', style: kSearchBtnsPanelStyle },
                    this.getDsSearchBtns()
                ),
                React.createElement(DsSearchGrid, {
                    dsid: this.props.dsid,
                    mid: this.props.mid,
            ...
            React.createElement(
                'div',
                { className: 'kAssisantPanel', style: kAssisantPanelStyle },
                React.createElement(SearchProgress, { dsid: this.props.dsid,
                    mid: this.props.mid,
                    ref: 'searchProgress'
                })
            ),
            ...
            React.createElement(DjPage, { ref: 'djPage',
                dsid: this.props.dsid,
                mid: this.props.mid,
                dataListClick: this.dataListClick
            })
...
var Module = k.react.Module = React.createClass({
    ...
    getDsSearchPages: function () {
        ...
        for (var i = 0, len = searchPages.length; i < len; i++) {
            ...
            DsSearchPages.push(React.createElement(DsSearchPage, { key: dsid, dsid: dsid, mid: this.props.mid, isSelect: isDsSelect,
    ...
    getTabBtns: function () {
        ...
        if (this.sysPages && this.sysPages.length > 0) {
            ...
            for (var i = 0, len = this.sysPages.length; i < len; i++) {
                ...
                //如果有第二个查询页,就把分割线放出来
                if (i >= 1) {
                    this.tabBtns.push(React.createElement('span', { key: "dstabSplit-" + this.sysPages[i]['DS_ID'],
                ...
                //把tab按钮加进来
                this.tabBtns.push(React.createElement(
                    'div',
                    { key: "dstab-" + this.sysPages[i]['DS_ID'],
                        className: className,
    ...
    render: function () {
        ...
        return React.createElement(
            'div',
            { id: this.props.mid, style: modulePanelStyle },
            React.createElement(
                'div',
                { style: this.DsTabPanelStyle },
                this.tabBtns
            ),
            DsSearchPages
        );
