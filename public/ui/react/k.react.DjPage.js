var GaiZhang = k.react.GaiZhang = React.createClass({
    ...
    render: function () {
        ...
        return <div id={this.props.dsid + "_gaizhang"} className="verifyGZ" style={outStyle}>
            <span style={btStyle}>{this.state.bt}</span>
            <span style={manStyle}>{this.state.man}</span>
            <span style={dateStyle}>{this.state.date}</span>
        </div>;
    }
});
var MxPage = k.react.MxPage = React.createClass({
    getMxPageBtns: function () {
        ...
        for (var i = 0, len = dsbutns.length; i < len; i++) {
            btn = dsbutns[i];
            MxDjBtns.push(
                <CommonButton key={'djPage' + btn['NAME']}
    ...
    btnClick: function (fn, event, proc, btid) {
        ...
        if (this.canEdit == true) {
            if (this[fn])
                return this[fn](event.target, proc, btid);
            else {
    ...
    render: function () {
        ...
        return <div className={className}>
            <div className="kMxDjBtnsPanel" style={kMxDjBtnsPanelStyle}>
                {this.getMxPageBtns()}
            </div>
...
k.react.DjPage = React.createClass({
    ...
    btnClick: function (fn, event, isProc, btid) {
        ...
        if (this[fn])
            return this[fn](event.target);
        else if (isProc == true)
            this.execProc(fn, btid, event.target);
        else
            $(event.target).removeClass('noneButton');
    },
    ...
    save: function (btn) {
        ...
        var isMxUp = false, mxUpdata;//判断明细是否修改过
        if (this.mxPage != undefined && this.mxPage.length > 0) {
            var dsid;
            for (var i = 0, len = this.mxPage.length; i < len; i++) {
                dsid = this.mxPage[i];
        ...
        var mainDjUp = (that.isInAdd == true)
            ? this.refs['mainDj'].GetDjAddData()
            : this.refs['mainDj'].GetDjUpData();//得到主表的修改数据
        //如果没有修改数据,就直接返回即可
        if (mainDjUp == 'noEdit' && isMxUp == false) {
            ...
            if (this.isInAdd == true)
                mainDjUp = {Add: [{}]};
            else
                mainDjUp = {};
        } else if (this.isInAdd == true
            && mainDjUp != 'noEdit'
            && this.mxPage != undefined
            && this.mxPage.length > 0
            && isMxUp == false
            && (this.mainDsId.toString().notIn('ds_productin', 'ds_yl_pd', 'ds_cominfo'))
        ) {
            alert('请录入明细数据!');
            if (btn != undefined) $(btn).removeClass('noneButton');
            return false;
        ...
        k.CallServerAPI('saveds', {
            ...
            if (k.isArray(data.saveCheck)) {
                var msg = "";
                for (var i = 0, len = data.saveCheck.length; i < len; i++)
                    msg += data.saveCheck[i].toString() + "\r\n";
                alert(msg);
                return;
            }
    ...
    addds: function (btn) {
        ...
        this.SetCanEdit(true);
        this.LoadGaiZhange();
        for (var i in this.refs) {////遍历明细,设定成可以编辑
    ...
    getMainDjBtns: function () {//下面的按钮
        ...
        for (var i = 0, len = mainDjBtns.length; i < len; i++) {
            if (mainDjBtns[i]['JS_FUNC'] == 'exportExcelByNode')
    ...
    selectTab: function (e) {
        ...
        this.jQuery.find('.kDsTabHeader').each(function () {
            if ($(this).attr('id') == e.target.id)
                $(this).addClass('kDsTabHeaderSelect');
            else
                $(this).removeClass('kDsTabHeaderSelect');
        });
    },
    getMxTabBtns: function () {
        ...
        for (var i = 0, len = this.mxPage.length; i < len; i++) {//遍历明细页
            className = "kDsTabHeader";
            if (i >= 1) {
                tabBtns.push(<span key={"dstabSplit-" + this.mxPage[i]}
        ...
        return <div style={DsTabPanelStyle}>{tabBtns}</div>;
    },
    getMxPages: function () {
        if (this.mxPage == undefined || this.mxPage.length == 0) return [];
        var mxPages = [];
    ...
    render: function () {
        ...
        return <div className="kDjPage" style={showStyle}>
            <GaiZhang dsid={this.props.dsid}
                      mid={this.props.mid}
                      ref={'verifyGZ'}
            />
            <div className="kMainDjPanel" style={kMainDjPanelStyle}>
                <div className="kMainDjBtnsPanel" style={kMainDjBtnsPanelStyle}>
                    {this.getMainDjBtns()}
                </div>
                <FormGrid
                    dsid={this.props.dsid}
