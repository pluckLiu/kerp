k.react.DataList = React.createClass({
    ...
    showDataList: function (dz_id, dzdata, theClickCell, hostCtrl, inMxPage, mustWcols) {
        ...
        if (this.inMxPage == 'inMxPage')
            this.POP_RTCOLS = dzdata['POP_RTCOLS'];
    ...
    sure: function (btn) {
        ...
        if (this.inMxPage == 'inMxPage') {
            ...
            if (sureCols.length > 0 && sureCols[0] != '') {
                var tmp, tcol, acol;
                for (var i = 0, len = selectRows.length; i < len; i++) {//遍历确定数组
                    tmp = selectRows[i];
    ...
    getDownBtns: function () {//下面的按钮
        var DsSearchBtns = [], btn;
        var CommonButton = k.react.CommonButton;
        var searchBtns = [
            {"NAME": 'close', "BUTN_TEXT": '关闭', "JS_FUNC": 'close'},
            {"NAME": 'sure', "BUTN_TEXT": '确定', "JS_FUNC": 'sure'},
            {"NAME": 'search', "BUTN_TEXT": '查询', "JS_FUNC": 'search'}];
        for (var i = 0, len = searchBtns.length; i < len; i++) {
            btn = searchBtns[i];
            DsSearchBtns.push(
                <CommonButton key={btn['NAME']}
                              dsid={this.props.dsid}
                              mid={this.props.mid}
                              text={btn['BUTN_TEXT']}
                              jsFn={btn['JS_FUNC']}
    ...
    render: function () {
        ...
        return <div className="kDataListMask" style={showStyle}>
            <div className="kDataList">
                <SearchGrid
                    dsid={this.props.dsid}
                    mid={this.props.mid}
