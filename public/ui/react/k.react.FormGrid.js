var DataCell = k.react.DataCell = React.createClass({
    getInitialState: function () {
        var state = {};
        state.where = '';
        if (this.props.parentCtrlType == 'SearchPage'
            && this.props.colName.indexOf('-') == -1 //在拿到这个列的时候如果本身就有 - 就不用管了,一般用于排除时间控件
        ) {
    ...
    setInput: function (text, val) {
        ...
        if (colOpt != undefined && colOpt['AFTER_EDIT'] != undefined && colOpt['AFTER_EDIT'] != '') {
            try {
                ...
                //如果是关联dz获取数据
                if (theEval.type == 'relateDzSet'
                    && theEval.relateDzId != ''
                    && theEval.relateDzCols != ''
                    && theEval.setCols != ''
                ) {
                    //post查询dz
                    var wcols = {};
                        ...
                        success: function (data) {
                            if (data != undefined && data.json != undefined && data.json.data != undefined) {
                                ...
                                if (dzSearch.length > 0) {
                                    ...
                                    var setCols = theEval.setCols.split(','), setColOpt;
                ...
                else if (theEval.type == 'relateDataGridSet'
                    && theEval.relateDsId != ''
                    && theEval.relateWCols != ''
                    && theEval.setDsId != ''
                ) {
                    //先获取所需要的查询条件，relateWCols 用 逗号 分割,用冒号 前面的是本身的列,后面的是用来做查询条件的列
                    var wdata = {}, wcols = theEval.relateWCols.split(','), that = this, keyCol, dataCol, dateColValue;
                    for (var i = 0, len = wcols.length; i < len; i++) {
                        dataCol = wcols[i].split(':');
                        if (dataCol.length == 1) {
                            keyCol = dataCol = dataCol[0];//如果没有冒号,就是同样的列了
                        } else if (dataCol.length == 2) {//如果有2个列,就是第一个列是值列,第二个列是条件列
                            keyCol = dataCol[1];
                            dataCol = dataCol[0];
                        } else {//如果有多个的话,就用第一个了
    ...
    dataListClick: function () {
        //读取配置看 是否有前置查询条件
        var colOpt = this.props.colOpt, dzwol = {};
    ...
    render: function () {
        ...
        var isMust = <span className="kMustSpan">{"*"}</span>;
        ...
        if (this.props.CanEdit == false) {
            return <div style={this.props.cellStyle}>
                <span className="kDataCellText">{spanText}</span>
                {isMust}
                <span className="kDataInputNoEdit" style={inputStyle}>{renderText}</span>
            </div>;
        } else if (this.props.ctrType == 'date') {//弹框控件,设定弹框对照信息
            return <div style={this.props.cellStyle}>
                <span className="kDataCellText">{spanText}</span>
                {isMust}
                <input className="kDataInput"
                       style={inputStyle}
                       value={renderText}
            ...
            return <div style={this.props.cellStyle}>
                <span className="kDataCellText">{spanText}</span>
                {isMust}
                <input className="kDataInput" type="text"
                       value={renderText}
                       onChange={this.inputChange}
                       onFocus={this.focus}
                       onKeyUp={this.keyUp}
...
k.react.FormGrid = React.createClass({
    ...
    JudgeMustEdit: function () {
        var allColOpt = this.allColOpt, colopt, colV = null;
        var rt = '';
        //遍历配置列,获取其对应的值
    ...
    GetColData: function (colName) {
        var gridData = this.mData, colData;
        if (gridData.Up != undefined)
            colData = gridData.Up[colName];
        if (colData == undefined)
            colData = gridData.data[colName];
        //对于时间的转换
    ...
    getFormCells: function () {
        ...
        //进行遍历添加
        var formCells = [], col, cellStyle,
            cellLeft = 0, cellTop = 0, cellWidth = 0,
            colIndex = 0, rowIndex = 0, cellText = '',
            gridData = null, allColOpt = {};
        ...
        for (var i = 0, len = formCols.length; i < len; i++) {
            ...
            formCells.push(
                <DataCell key={col['NAME']}
                          ...
                          CanEdit={ this.props.parentCtrl == 'SearchPage' ? true
                              : this.state.CanEdit == false ? false
                                  : col['IS_EDIT'] == 'F' ? false
                                      : true }
