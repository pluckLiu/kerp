    ...
    render: function () {
        ...
        if (this.props.canUse == false) {
            return React.createElement(
                'div',
                { style: this.props.cellStyle, className: this.props.cellClass
                },
                renderText
            );
        } //如果用都不能用,就直接返回最简单的
        ...
        if (this.props.dzid == null || this.props.dzid == '') {
            //常规非对照信息,就用input 承载即可
            return React.createElement(
                'div',
                { style: this.props.cellStyle, className: this.props.cellClass },
                React.createElement('input', { className: 'kDataGridInput', type: 'text',
                    value: renderText,
                    onChange: this.inputChange,
                    onFocus: this.focus
                }),
                React.createElement('span', { className: 'kDataGridEditImage' })
            );
        } else if (this.props.ctrType == 'image') {
            //图形控件
            return React.createElement(
                'div',
                { style: this.props.cellStyle, className: this.props.cellClass },
                React.createElement('span', { className: renderText, onClick: this.selectClick })
            );
...
var DataGridRow = k.react.DataGridRow = React.createClass({
    ...
    render: function () {
        ...
        //如果有 鼠标 进入的背景色,就赋予这个
        if (this.state.isMouseEnter == true) rowStyle.backgroundColor = "#ebeced";
        //选中状态必须是这个颜色
        if (this.state.isSelect == true) rowStyle.backgroundColor = '#93dbb0';
        return React.createElement(
            'div',
            { className: 'kGridRow', style: rowStyle,
...
var DgSelect = k.react.DgSelect = React.createClass({
    ...
    render: function () {
        var theClass;
        if (this.state.isSelect == true) theClass = "kDataGridSelect kSelectImage";else theClass = "kDataGridSelect";
        return React.createElement('div', { className: theClass, onClick: this.props.selectClick });
    }
});
var LeftGridRow = k.react.LeftGridRow = React.createClass({
    ...
    render: function () {
        ...
        if (this.state.isMouseEnter == true) leftRowStyle.backgroundColor = "#ebeced";
        if (this.state.isSelect == true) {
            leftRowStyle.backgroundColor = '#93dbb0';
        }
        return React.createElement(
            'div',
            { className: 'kGridRow kRowNo',
                style: leftRowStyle,
                onClick: this.dgCellClick,
                onMouseEnter: this.onMouseEnter,
                onMouseLeave: this.onMouseLeave
            },
            React.createElement(DgSelect, { ref: 'select',
            ...
            React.createElement(
                'div',
                { className: 'kDataGridCell kLeftTopDgRowNo',
                    onClick: this.dgCellClick },
                (this.props.rowno + 1).toString()
            )
        );
    }
});
var DataGrid = k.react.DataGrid = React.createClass({
    ...
    DelAllSelect: function () {
        ...
        for (var i in data.Select) {
            //遍历 select 然后看对应的gridData 是否在add中存在
            if (data.Select[i] == true) {
                //只有为真的情况 才算是选中
                if (k.ArrayIndexV(data.EditData['Add'], gridData[i]) == -1) {
                    pkVal = gridData[i][body.pkCol];
                    if (pkVal != '' && pkVal != undefined) data.EditData['Del'].push(pkVal);
    ...
    AddEditData: function (addData) {
        var data = this.mData,
            body = { data: addData, allColOpt: this.allColOpt };
        data.EditData = data.EditData == undefined ? {} : data.EditData; //确保 存在EditData
        data.EditData['Add'] = data.EditData['Add'] == undefined ? [] : data.EditData['Add'];
        //添加的数据中,对应可以编辑的列要赋予一个初始值
        var allColOpt = body.allColOpt,
            colOpt,
            tmpRow;
    ...
    GetUpData: function () {
        ...
        if ((editData['Up'] == undefined || editData['Up'].length == 0) && (editData['Add'] == undefined || editData['Add'].length == 0) && (editData['Del'] == undefined || editData['Del'].length == 0)) return 'noEdit';
        return editData;
    },
    GetSelectRows: function (sort) {
        ...
        for (var i in select) {
            if (select[i] == true) selectRows.push(GridData[i]);
        }
        return selectRows;
    },
    GetSelectRowsPk: function (sort) {
        ...
        if (sort == 'asc' || sort == undefined) {
            ...
            for (var i = 0, len = theSelect.length; i < len; i++) {
                ...
                } else selectRows.push({ pkid: GridData[theSelect[i]][this.pkCol] });
            }
        } else if (sort == 'desc') {
            var theSelect = [];
            for (var i in select) select[i] == true ? theSelect.push(i) : null;
        ...
        for (var r = 0, rlen = gridData.length; r < rlen; r++) {
            ...
            for (var i = 0, len = gridCols.length; i < len; i++) {
                ...
                if (k.isNumber(cellText)) {
                    //如果是数值型的话,把小数点保留3位,之所以到这里才判断,是尽量把 continue的情况都考虑完
    ...
    calcGridStyle: function () {
        ...
        for (var i = 0, len = gridCols.length; i < len; i++) {
            ...
            gridStyle += '.' + this.kClass + '_' + col['NAME'] + "{width:" + cellWidth.toString() + "px;" + "left:" + cellLeft.toString() + "px;}\r\n";
        ...
        for (var i = 0, len = gridCols.length; i < len; i++) {
            ...
            headerCells.push(React.createElement(
                'div',
                { key: col['NAME'], className: cellClassName },
                col['TEXT']
            ));
        }
        //最后进行渲染
    ...
    calcGridCell: function () {
        ...
        var currentRow = Math.floor(scrollTop / 25),
            //25行高一行,当为 0-25 高度,其实行为 0行, 除以25的左边整数即是实际行数
        endRow = currentRow + fastRowLen,
            //结束的行号
        rowStyle,
            //行样式
        leftRowStyle,
        ...
        for (; currentRow < endRow; currentRow++) {
            //开始生成渲染数数据,从当前行一直到结束行
            rowData = renderData[currentRow]; //获取当前行数据
            rowCells = []; //重置单元格数组
            cellLeft = 0; //单元格左边距离
            canRowEdit = true; //本行后面的列是否可以编辑
            for (var i = 0, len = this.gridColsOpt.length; i < len; i++) {
                //遍历单元格配置
                ...
                if (rowData == undefined || rowData[colOpt['NAME']] === undefined) {
                    ...
                    rowCells.push(React.createElement(
                        'div',
                ...
                if (this.cellRender[colOpt['NAME']] && k.isObject(this.cellRender[colOpt['NAME']]['json']) && this.cellRender[colOpt['NAME']]['json']['renderRow'] != undefined) {
                    try {
                        //传入实际数据,并用 三目表达式来判断后续的单元格是否可以编辑
                        canRowEdit = eval('(' + this.cellRender[colOpt['NAME']]['json']['renderRow'].replace('$val', cellText) + ')');
                    } catch (e) {
                ...
                rowCells.push(React.createElement(DgInputCell, {
                    key: currentRow.toString() + '.' + colOpt['NAME'],
                    rowno: currentRow,
                    ...
                    canEdit: this.state.CanAllEdit == false ? false : colOpt['IS_EDIT'] == 'T' && this.state.hasData == true && canCellNowEdit == true ? true : false,
            ...
            var isSelect = this.state.canUse == false ? false : this.selectRows[currentRow] == undefined ? false : this.selectRows[currentRow];
            //单元格生成完以后,就放入到数据行中
            /*if(currentRow<3)
             console.info("currentRow,isSelect",currentRow,isSelect,this.selectRows);*/
            this.dataRows.push(React.createElement(DataGridRow, { className: 'kGridRow',
                rowStyle: rowStyle,
            ...
            if (!this.state.hasData || rowData == undefined) {
                this.leftRows.push(React.createElement(
                    'div',
                    { className: 'kGridRow kRowNo', key: currentRow.toString() + 'kleftRow',
                        style: leftRowStyle },
                    ""
                ));
            } else {
                this.leftRows.push(React.createElement(LeftGridRow, { key: currentRow.toString() + 'kleftRow',
    ...
    mouseEnterCell: function (rowno, colName) {
        if (this.refs["dataGridRow" + rowno.toString()] != undefined) this.refs["dataGridRow" + rowno.toString()].setState({ isMouseEnter: true });
        if (this.refs["leftGridRow" + rowno.toString()] != undefined) this.refs["leftGridRow" + rowno.toString()].setState({ isMouseEnter: true });
    },
    mouseLeaveCell: function (rowno, colName) {
        if (this.refs["dataGridRow" + rowno.toString()] != undefined) this.refs["dataGridRow" + rowno.toString()].setState({ isMouseEnter: false });
        if (this.refs["leftGridRow" + rowno.toString()] != undefined) this.refs["leftGridRow" + rowno.toString()].setState({ isMouseEnter: false });
    },
    dgCellClick: function (rowno, colName) {
        if (this.state.isMultiSelecct == false && k.isNumber(this.lastSelect)) {
            if (this.refs["dataGridRow" + this.lastSelect.toString()] != undefined) this.refs["dataGridRow" + this.lastSelect.toString()].setState({ isSelect: false });
            if (this.refs["leftGridRow" + this.lastSelect.toString()] != undefined) this.refs['leftGridRow' + this.lastSelect.toString()].setState({ isSelect: false });
        } //如果不多选的,就把上一次选中设置为不选中
        //触发行选中
        var data = this.mData,
            isMultiSelecct = this.state.isMultiSelecct;
        data.Select = data.Select == undefined ? {} : data.Select;
        ...
        if (colOpt != undefined && colOpt['IS_DIG'] != undefined && (colOpt['IS_DIG'] == 'T' || colOpt['IS_DIG'].toString().indexOf(':') > 0) && k.isFunction(this.props.digDsEditPage)) {
            //判断是否为挖掘时间
            var that = this;
    ...
    selectAllRows: function () {
        ...
        if (k.isArray(data.GridData)) {
            if (isSelectAllRow == false) {
                ...
                for (var i = 0, len = data.GridData.length; i < len; i++) data.Select[i] = true;
            }
    ...
    cellChange: function (rowno, colName, value, showText) {
        ...
        if (this.cellRender[colName] && k.isObject(this.cellRender[colName]['json'])) {
            ...
            if (cellRenderJson.jsFns != undefined) {
                ...
                , this.theDataId, cellRenderJson['jsFns'], {
                    val: value,
                    text: showText,
                    col: colName,
                    rowno: rowno,
    ...
    render: function () {
        ...
        if (this.state.isMultiSelecct == true) {
            leftTopArea = React.createElement(
                'div',
                { className: 'kDataGridLeftTopArea', style: this.leftTopAreaStyle },
                React.createElement(
                    'div',
                    { className: 'kHeaderCellText kLeftTopDgSelectCol', onClick: this.selectAllRows },
                    '\u5168\u9009',
                    React.createElement('span', { className: 'kLeftTopDgSelect' })
                ),
                React.createElement(
                    'div',
                    { className: 'kHeaderCellText kLeftTopDgRowNo' },
                    '\u5E8F\u53F7'
                )
            );
        } else {
            leftTopArea = React.createElement(
                ...
                React.createElement(
                    'div',
                    { className: 'kHeaderCellText kLeftTopDgSelectCol' },
                    React.createElement('span', { className: 'kLeftTopDgSelect' })
                ),
                React.createElement(
                    'div',
        ...
        return React.createElement(
            'div',
            { className: 'kDataGridPanel', style: this.props.topStyle },
            React.createElement(
                'style',
                null,
                this.gridStyle
            ...
            React.createElement(
                'div',
                { className: 'kDataGridHeaderRow', style: this.headerRowStyle },
                this.headerCells
            ),
            React.createElement(
                'div',
                { className: 'kLeftArea', style: this.leftAreaStyle },
                this.leftRows
            ),
            React.createElement(
                'div',
                { className: 'kDataArea', style: this.dataAreaStyle },
                React.createElement(
                    'div',
                    { className: 'kDataDiv' },
                    React.createElement('div', { className: 'kLineVercital', style: this.lineVercitalStyle }),
                    React.createElement(
                        'div',
                        { className: 'kFastData', style: this.fastDataStyle },
                        this.dataRows
                    )
                )
            )
